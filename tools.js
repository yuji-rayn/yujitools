...
function promptMenu() {
  ...
  console.log("║ { 19  } Skip URL (Mencegah Terkena Phising Iklan)".padEnd(53) + "║");
...
async function extractFinalUrl(intermediateUrl) {
  try {
    // Check if already a direct file host URL
    const directHosts = ['mediafire.com', 'google.com', 'dropbox.com', 'mega.nz'];
    if (directHosts.some(host => intermediateUrl.includes(host))) {
      return intermediateUrl;
    }

    const parsedUrl = new URL(intermediateUrl);
    const queryParams = new URLSearchParams(parsedUrl.search);

    // Check for 'u' parameter (common in URL shorteners)
    if (queryParams.has('u')) {
      const encodedUrl = queryParams.get('u');
      return decodeUrl(encodedUrl);
    }

    // Check other common URL parameters
    const urlParams = ['url', 'link', 'target', 'redirect', 'dest'];
    for (const param of urlParams) {
      if (queryParams.has(param)) {
        return decodeUrl(queryParams.get(param));
      }
    }

    // If no parameters found, return original URL
    return intermediateUrl;
  } catch (error) {
...
function decodeUrl(encodedStr) {
  try {
    // First decode URI components (%20, %3D, etc.)
...
async function urlScraper(url) {
  try {
    ...
    const finalUrl = await extractFinalUrl(url);
    ...
    const filename = `url_scraper_${Date.now()}.txt`;
    const filePath = path.join(OUTPUT_DIR, filename);
    const content = `URL Scraper Result\n==================\nOriginal URL: ${url}\nFinal URL: ${finalUrl}\nTimestamp: ${new Date().toISOString()}`;
...
function urlScraperMenu() {
  console.clear();
  console.log("\n🔗 SKIP URL/LINK");
  console.log("1. 🔍 SKIP URL/LINK");
  console.log("2. 🔙 Kembali ke Menu Utama");

  rl.question("Pilih opsi: ", (choice) => {
    if (choice === "1") {
      rl.question("\n🔗 Masukkan URL yang ingin diskip\n(Contoh: https://sfl.gl/ready/go?u=aHR0cHM6Ly93d3cubWVkaWFmaXJlLmNvbS9maWxlL3JkZW5xNHJkcWVtNWJkbS9FdWRvcmFfS3VudGlsYW5ha19ieV8lNDBtbGJidWxsLnppcC9maWxl&t=cGFnZTN3aXRoYWRzZW5zZQ%3D%3D): ", async (url) => {
        if (!url) return urlScraperMenu();
        
        try {
          await urlScraper(url);
        } catch (error) {
          console.error("❌ Error:", error.message);
        }
